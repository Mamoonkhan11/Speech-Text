# Main application file for speakEasy AI - Streamlit app for audio transcription using Whisper.
...
from modules.record import start_recording, stop_recording, SAMPLE_RATE, SOUND_ENABLED
...
# Language selector
language = st.selectbox( "🌐 Choose Language", sorted([ "Afrikaans", "Albanian", "Amharic", "Arabic", "Armenian", "Assamese", "Azerbaijani", "Bashkir", "Basque", "Belarusian", "Bengali", "Bosnian", "Bulgarian", "Burmese", "Catalan", "Chinese", "Croatian", "Czech", "Danish", "Dutch", "English", "Estonian", "Finnish", "French", "Galician", "Georgian", "German", "Greek", "Gujarati", "Haitian Creole", "Hausa", "Hawaiian", "Hebrew", "Hindi", "Hungarian", "Icelandic", "Indonesian", "Italian", "Japanese", "Javanese", "Kannada", "Kazakh", "Khmer", "Kinyarwanda", "Korean", "Lao", "Latin", "Latvian", "Lithuanian", "Luxembourgish", "Macedonian", "Malagasy", "Malay", "Malayalam", "Maltese", "Maori", "Marathi", "Mongolian", "Nepali", "Norwegian", "Odia", "Pashto", "Persian", "Polish", "Portuguese", "Punjabi", "Romanian", "Russian", "Samoan", "Serbian", "Shona", "Sindhi", "Sinhala", "Slovak", "Slovenian", "Somali", "Spanish", "Sundanese", "Swahili", "Swedish", "Tagalog", "Tajik", "Tamil", "Tatar", "Telugu", "Thai", "Turkish", "Ukrainian", "Urdu", "Uzbek", "Vietnamese", "Welsh", "Xhosa", "Yiddish", "Yoruba", "Zulu" ]) )

# Input method selector
input_method = segmented_method(options=["Speak", "Upload File"], default="Speak")

# Speak selection section
# ───────────────────────────────────────────────────────────────
if input_method == "Speak":
    if not SOUND_ENABLED:
        st.warning(" Microphone recording is not supported on Streamlit Cloud. Please upload an audio file instead.")
    else:
        mic_glow()
        ...
        # After recording is done
            ...
            if not os.path.exists(st.session_state.recorded_path) or os.path.getsize(st.session_state.recorded_path) == 0:
                st.error(" Recording file missing or empty. Please record again.")
            else:
                ...
                with c1:
                    if st.button(" Transcribe Audio"):
                        ...
                        if text:
                            ...
                            st.error(" Failed to transcribe audio. Please check your recording.")
    ...
    if st.session_state.text:
        ...
        st.download_button(
            " Download Transcription Report (.txt)",
    ...
    if up:
        ...
        if st.button(" Transcribe Uploaded File"):
            ...
            if text:
                ...
                st.error(" Could not process uploaded audio file.")
